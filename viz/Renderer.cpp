...
void Renderer::setWorldSize(float wW, float wH)
{
    worldW = wW;
    worldH = wH;
    resetQuery();
...
void Renderer::setScenarioInfo(const std::string& distribution,
                               int capacity,
                               float minRadius,
                               float maxRadius,
                               float maxSpeed)
{
    ...
    scenarioMinRadius = minRadius;
    scenarioMaxRadius = maxRadius;
    scenarioMaxSpeed = maxSpeed;
}
                       ...
                       bool paused)
{
    ...
    kv("Distribucion:", distributionName.c_str(), C_TXT_BLUE);
    snprintf(buf, sizeof(buf), "%.0fx%.0f", worldW, worldH);
    kv("Espacio:", buf, C_TXT);
    snprintf(buf, sizeof(buf), "%d", scenarioCapacity);
    kv("Capacidad QT:", buf, C_TXT);
    snprintf(buf, sizeof(buf), "%.1f-%.1f", scenarioMinRadius, scenarioMaxRadius);
    kv("Radios:", buf, C_TXT);
    snprintf(buf, sizeof(buf), "%.0f", scenarioMaxSpeed);
    kv("Vel. max:", buf, C_TXT);
    ...
    if (showHelp)
    {
        ...
        key("[1][2][3]", " Uniforme / clusters / alta densidad");
        key("[N]", " Cambiar cantidad de particulas");
        key("[M]", " Cambiar tamano del mundo");
        key("[K]", " Cambiar capacidad del QT");
        key("[T]", " Cambiar rango de radios");
        key("[V]", " Cambiar velocidad maxima");
        key("[ENTER]", " Regenerar escenario");
